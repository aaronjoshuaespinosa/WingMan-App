...
const createAppointment = async (req, res) => {
    const {title, description, type, status, email, fullName, username, studentNumber} = req.body;
    try {
        const user_id = req.user._id;
        const appointment = await Appointment.create({title, description, type, status, user_id, email, fullName, username, studentNumber});
        res.status(200).json(appointment);
    }
    catch (error) {
